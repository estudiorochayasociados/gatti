...
function Categoria_Read_Productos_Botones($tipo) {
  $idConn = Conectarse();
  $sql = "SELECT categoria_portfolio FROM portfolio WHERE tipo_portfolio = '$tipo' GROUP BY categoria_portfolio";
  $resultado = mysql_query($sql, $idConn);
  while ($row = mysql_fetch_array($resultado)) {
    if($row["categoria_portfolio"] != "ACCESORIOS") {
      ?>
      <a href="?buscar=<?php echo $row["categoria_portfolio"] ?>"><?php echo strtoupper($row["categoria_portfolio"]); ?></a>   
      <hr/>   
      <?php
    } 
  }
...
function Slider_Read() {
  ...
  while ($row = mysql_fetch_array($resultado)) { 
    ...
    ?>
    <div class="item <?php if($i == 0) {echo "active";} ?>" style="background:url('<?php echo BASE_URL; ?>/<?php echo $row["ImgSlider"] ?>') center center/cover;height:520px">
...
function Portfolio_Read_Front($tipo,$categoria) {
  ...
  if($categoria == '') {
    $query = "SELECT * FROM  `portfolio` where estado_portfolio = 0 and tipo_portfolio = '$tipo'   ORDER BY RAND()";
    $res = $con->query($query);
    ...
    $consulta = "SELECT * FROM  `portfolio` where estado_portfolio = 0 and tipo_portfolio = '$tipo'   ORDER BY RAND()
    LIMIT " .(($paginacion->get_page() - 1) * $resul_x_pagina). ',' .$resul_x_pagina;
  } else {
    $query = "SELECT * FROM  `portfolio` where categoria_portfolio = '$categoria' and estado_portfolio = 0 and tipo_portfolio = '$tipo'   ORDER BY RAND()";
    $res = $con->query($query);
    $num_registros = mysqli_num_rows($res);
    $resul_x_pagina = 12;
    $paginacion = new Zebra_Pagination();
    $paginacion->records($num_registros);
    $paginacion->records_per_page($resul_x_pagina);
    $consulta = "SELECT * FROM  `portfolio` where categoria_portfolio = '$categoria' and estado_portfolio = 0 and tipo_portfolio = '$tipo'   ORDER BY RAND()
    LIMIT " .(($paginacion->get_page() - 1) * $resul_x_pagina). ',' .$resul_x_pagina;
  }
  ...
  while ($row = mysqli_fetch_array($result)) {
    ...
    <div class=" col-md-4 col-xs-12 product-box hvr-outline-in"> 
      <a href="producto.php?id=<?php echo strtoupper($row["id_portfolio"]) ?>">
      ...
      <h1 class="tituloProducto"><a href="producto.php?id=<?php echo $row["id_portfolio"] ?>"  style="font-size:25px"><?php echo ($titulo); ?></a></h1>
      ...
      if($tipo != 0) {
        ?>
        <span class="precioProducto"><?php echo "<b>Precio: </b>$".($precio) ?></span>
        <?php  
      } 
      ?>
      <div class="clearfix"></div><br/>
      <a href="producto.php?id=<?php echo $row["id_portfolio"] ?>" class="hvr-shutter-out-vertical btn btn-default"  ><i class="fa fa-plus"></i> VER MÁS</a>
    </div> 
    <?php
  }
...
function Portfolio_Read_Slide() {
  ...
  <div class="productosMasVistos">
    ...
    <h1 class="tituloProducto"><a href="producto.php?id=<?php echo $row["id_portfolio"] ?>"><?php echo  ($titulo) ?></a></h1>
    <span class="precioProducto"><b>Categoría: </b><br/><a href='productos.php?buscar=<?php echo $row["categoria_portfolio"] ?>'><?php echo $row["categoria_portfolio"] ?></a></span><br/>

    <div class="clearfix"></div><br/>
    <a href="producto.php?id=<?php echo $row["id_portfolio"] ?>" class="btn btn-default"  ><i class="fa fa-plus"></i> VER MÁS</a>
  </div>
  <?php
}
}
  ...
  while ($row = mysql_fetch_array($resultado)) {
    ?>
        ...
        <div class="thumbnail" style="height:400px">
          ...
          <a href="producto.php?id=<?php echo $row["id_portfolio"] ?>" title="<?php echo utf8_encode($row["nombre_portfolio"]) ?>" ><div style="background: url(<?php echo $row["imagen1_portfolio"] ?>) center center;background-size:cover;height:200px;"></div></a>
          <div class="caption">
            <p><a href="producto.php?id=<?php echo $row["id_portfolio"] ?>" title="<?php echo utf8_encode($row["nombre_portfolio"]) ?>" class="btn btn-primary2" style="background: #E64A50" role="button">Ver más</a><br /><br />
    ...
    function Notas_Read_Front_Side() {
      ...
      while ($row = mysql_fetch_array($resultado)) {
        ...
        <li class="notasSide row">
          ...
          <div class="col-md-8"><a href="notas.php?id=<?php echo $row["IdNotas"] ?>" style="text-transform: uppercase "><?php echo substr(strtoupper(htmlspecialchars($row["TituloNotas"])),0,40)."..." ?></a><br/>
          ...
          <div class="col-md-4">
            <div class="notasMasVistos">
              <span style="position:relative">
                <a href="notas.php?id=<?php echo $row["IdNotas"] ?>"><div style="background:url('<?php echo $imagen ?>') no-repeat center center;background-size:cover;height:200px;width:100%"></div></a>
          ...
          <h5 class="panel-title" >
            <a data-toggle="collapse" data-parent="#accordion" href="#collapse<?php echo $row["id_pedidos"]; ?>" aria-expanded="true" aria-controls="collapse<?php echo $row["id_pedidos"]; ?>" >
             ...
             switch($row["estado_pedidos"]) {
              ...
              $leyenda = "<b>Estaríamos necesitando el pago para poder proseguir con el pedido solicitado.</b><br/>Por favor comunicarse a:<br/><br/>
              <b>CASA CENTRAL </b><br/>
              ...
              comercioexterior@gattisa.com.ar<br/>";
              break;
              case 1:
              echo "<span  style='padding:4px;font-size:13px;margin-top:-3px' class='btn-success fRight'>Estado: Pago exitoso</span>";
              $leyenda = "<b>Excelente el pago fue acreditado, para poder proseguir con el pedido solicitado.</b><br/>Por favor comunicarse a:<br/><br/>
              <b>CASA CENTRAL </b><br/>
              Rosario de Santa Fe 298 / San Francisco / Cba.<br/>
              Tel.: (03564) 420619 y lineas rotativas<br/>
              Fax: (03564) 421423<br/>
              ...
              $leyenda = "<b>Tuvimos problemas con el pago para poder proseguir con el pedido solicitado.</b><br/>Por favor comunicarse a:<br/><br/>
              <b>CASA CENTRAL </b><br/>
              Rosario de Santa Fe 298 / San Francisco / Cba.<br/>
              Tel.: (03564) 420619 y lineas rotativas<br/>
              Fax: (03564) 421423<br/>
